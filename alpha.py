...
zonas = pd.read_excel('Matrices/Códigos de Zonas.xls')
...
#%%
def trans_lista_a_df(lista):
     """Lee una lista de biblioteca con información de cargas, origen, destino y distancia
     y la transforma en un DataFrame.
     """
     df_derivabilidad = pd.DataFrame(data=(np.zeros((123,123))), index=(range(1,124)), columns=(range(1,124)))
     for x in lista:
         q = (x['ID destino'])
         w = (x['ID origen'])
         df_derivabilidad.xs(w)[q] = x['Carga']
             
     return df_derivabilidad
 
#%% GRANOS
def crear_matriz_carga_derivable(ruta_matriz_xls, pestaña, pestaña_criterio):
    """Convina todas las funciones antes declaradas para calcular la derivavilidad
    ...
    """    
    df_camion = trans_xlmatriz_a_df(ruta_matriz_xls, pestaña)
...
# matriz_granos_derivable = crear_matriz_carga_derivable('Matrices/Matrices Grupo Granos.xlsx','Total Toneladas Granos 2014','GRANOS')
...
# matriz_semiterminados_derivable = crear_matriz_carga_derivable('Matrices/Matrices Grupo Semiterminados.xlsx', 'Total Semiterminados 2014','SEMITERMINADOS')
# matriz_industrializados_derivable = crear_matriz_carga_derivable('Matrices/Matrices Grupo Industrializados.xlsx', 'Total Industrializados 2014','INDUSTRIALIZADOS')
# matriz_regionales_derivable = crear_matriz_carga_derivable('Matrices/Matrices Grupo Regionales.xlsx', 'Matriz Total Regionales 2014','MINERIA')
...
# trans_matriz_a_xlsx(matriz_derivable, 'Matrices/Derivabilidad.xlsx')
